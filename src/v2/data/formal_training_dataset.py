...
class FormalEpisode:
    ...
    def macro_transition_count(self) -> int:
        return math.ceil(self.step_count / 5)


class FormalTrainingDataset:
    def __init__(self, power_root: Path, ais_root: Path, power: pd.DataFrame, ais: pd.DataFrame) -> None:
    ...
    @classmethod
    def open(cls, power_root: Path, ais_root: Path) -> "FormalTrainingDataset":
        ...
        if not power_manifest.is_file() or not ais_manifest.is_file():
            raise FileNotFoundError("formal power/AIS manifest is missing")
        ...
        if required_power.difference(p.columns) or required_ais.difference(a.columns):
            raise ValueError("formal power/AIS manifest schema mismatch")
        if p["sample_id"].duplicated().any() or a["sample_id"].duplicated().any():
        ...
        if not identities.equals(ais_identities):
        ...
        result = cls(power, ais, p, a)
        if result.train_supervisory_steps != EXPECTED_TRAIN_STEPS:
            raise ValueError("formal Train supervisory-step count differs")
        if result.train_macro_transitions != EXPECTED_TRAIN_MACROS:
            raise ValueError("formal Train macro-transition count differs")
        return result
    ...
    def train_macro_transitions(self) -> int:
        return int(
            sum(
                math.ceil(int(value) / 5)
                for value in self._ais.loc[
                    self._ais["split"].eq("train"), "supervisory_row_count"
                ]
            )
        )
    ...
    def _load_episode(self, power_row: object, ais_row: object) -> FormalEpisode:
        ...
        if len(selected) != int(ais_row.supervisory_row_count) or len(speed) != len(selected):
            raise ValueError("power/AIS supervisory row counts differ")
        ...
        if not np.array_equal(selected_time, speed_time):
            raise ValueError("power/AIS supervisory axes differ")
        ...
        if power_timestamp.isna().any() or speed_timestamp.isna().any() or not np.array_equal(
            power_timestamp.astype(str).to_numpy(), speed_timestamp.astype(str).to_numpy()
        ):
            raise ValueError("power/AIS timestamps differ")
        ...
        if not np.isfinite(load).all() or not np.isfinite(velocity).all() or (velocity < 0.0).any():
            raise ValueError("formal power/AIS payload contains invalid values")
    ...
    def load_split(self, split: str) -> tuple[FormalEpisode, ...]:
        ...
        episodes = tuple(
            self._load_episode(row, ais_by_id.loc[str(row.sample_id)])

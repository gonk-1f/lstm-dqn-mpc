"""DEPRECATED: MPC entrypoint for invalid linear 1 s reconstruction.

DEPRECATED: this entrypoint consumes LSTM forecasts trained on 30 s to 1 s
linear-interpolation outputs. The reconstructed labels are non-causal and must
not be used as valid online high-frequency forecasting/control evidence.
"""
...
DEFAULT_LSTM_CKPT = PROJ / "outputs" / "lstm_total_load_1s_721" / "checkpoints" / "best_lstm_load_predictor.pt"
...
DEFAULT_OUT_DIR = PROJ / "outputs" / "lstm_mpc_total_load_1s_test"
DEFAULT_WEIGHT_SET = base.TOTAL_LOAD_QSOC400_QBATT003_QRAMP2E5_WEIGHT_SET
LOAD_DEFINITION = "fuel_cell_total_kw + battery_total_kw"
CAPACITY_BASIS = "full ship energy storage capacity for 1 s energy-side equivalent total load"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run fixed LSTM-MPC on 1 s total-load test voyages.")
    parser.add_argument("--weight_set", default=DEFAULT_WEIGHT_SET)
...
def configure_base_module(args: argparse.Namespace) -> None:
    ...
    base.DT_SECONDS = sample_interval_seconds
    base.MPC_BASE_KWARGS["dt_hours"] = sample_interval_seconds / 3600.0

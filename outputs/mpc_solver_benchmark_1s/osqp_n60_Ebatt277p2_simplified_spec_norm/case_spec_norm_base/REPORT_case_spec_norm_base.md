...
- Output directory: `outputs\mpc_solver_benchmark_1s\osqp_n60_Ebatt277p2_simplified_spec_norm\case_spec_norm_base`
...
- Weights: q_h2=`1.0`, q_soc=`1.0`, q_batt=`0.05`, q_ramp=`0.0`, q_terminal=`0.0`
- Max steps per voyage: `None`

## Solver Timing

- success_rate: `0.983716`
- infeasible_count: `1370`
- fallback_count: `0`
- mean / median / p90 / p95 / p99 / max solve time: `0.436` / `0.193` / `0.899` / `1.126` / `2.516` / `96.372` ms
- real_time_factor_mean: `0.000436`
- real_time_factor_p99: `0.002516`
- 1 s real-time gate passed: `False`

## Control Metrics

- total_h2_cost / H2_total_kg: `393.873189`
- SOC min/max/initial/final/final_minus_initial_mean: `0.458934` / `0.549955` / `0.550000` / `0.493641` / `-0.056359`
- SOC final_minus_initial_min by voyage: `-0.078936`
- SOC mean_abs_deviation: `0.051484`
- battery throughput/discharge/charge: `2446.845672` / `157.303875` / `2289.541797` kWh
- P_batt mean_abs/max/min: `96.246197` / `138.545583` / `-138.646190` kW
- P_batt abs<=1 kW fraction: `0.094414`
- P_fc mean/max/min: `284.756969` / `560.057450` / `-0.000137` kW
- fc_ramp_max / ramp_violation_count: `48.022144` kW/step / `2`
- power_balance_violation_max: `0.004162` kW
- SOC/battery/FC power violation counts: `0` / `0` / `0`
- battery/FC power violation max residuals: `0.046190` / `0.057450` kW; count tolerance `0.1 kW`
...
- solver_success_rate=0.983716, infeasible_count=1370
- timing gate failed: mean=0.436 ms, p95=1.126 ms, p99=2.516 ms, max=96.372 ms
- constraint check failed: balance_max=0.004162 kW, SOC_count=0, battery_count=0, FC_count=0, ramp_count=2
- SOC_final_minus_initial_min=-0.078936 < -0.02
- fraction(abs(P_batt)>=95% limit)=0.483403 > 0.05

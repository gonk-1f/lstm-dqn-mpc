...
- `P_fc_max = 560.0 kW`.
- `P_batt_ref = 138.6 kW`, matching the scaled 0.5C power for `E_batt = 277.2 kWh`.
- `SOC_band = 0.05`.
...
## Why 138.6 kW Replaces 350 kW

- The specification-book full system is about `1806 kWh` and about `900 kW`, which is about `0.5C`.
- Under the current scaled framework capacity, `277.2 kWh * 0.5 = 138.6 kW`.
- Therefore `350 kW` is retained only as a legacy assumption in historical outputs, not as the formal battery bound or normalization denominator for this run.

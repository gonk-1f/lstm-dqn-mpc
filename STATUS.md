...
| `SOC_band` | 候选 A/B/D 为 0.05，C 为 0.075 | 仅本轮固定候选实验 |
| FC ramp | 48 kW/s | QP 中为硬约束 |
| LSTM | history 30, prediction 6 | direct multi-output |
| 固定权重选择 horizon | `N=6` | 离线 ideal foresight runner 已实现；四组候选均拒绝 |
| historical benchmark horizon | `N=60` | 仅历史 solver/performance benchmark |

当前没有保留的固定权重。A–D 四组均固定 `q_h2=0.5`、`q_ramp=0`、`q_terminal_soc=0`，并按预注册参数运行；没有任何一组通过物理/SOC 门禁，因此没有 provisional 或 accepted 配置。
...
- **QP-MPC/OSQP：** 归一化凸 QP、等价仿射数值缩放、固定稀疏结构、边界更新、warm start、冷重启诊断、失败终止、`N=6` ideal-foresight runner 和历史 `N=60` benchmark 已存在。
...
- 已核对的 CLI 入口均可显示 `--help`；本轮完整运行了 N=6 A–D 四候选，但没有重跑训练或历史 N=60 benchmark。
- 本轮最终 `python -m compileall src` 通过，完整单元测试为 160 项全部通过；10 ms audit 仍严格校验 assignment key 必须恰为 train/validation/test。

## Provisional items

- 当前没有 provisional 固定权重；`configs/benchmarks/mpc_1s_n6_provisional.*` 未创建。
...
2. 指定 A–D 固定权重全部未通过 7 个 test 航次的物理/SOC 验收；在另行授权重设计并选出合格固定基线前不得训练正式 DQN。
...
1. **P0：** 基于本轮负结果另行设计固定 `N=6` MPC（必须先明确允许调整的权重范围或控制结构），不得自行增加第五候选。
...
- 状态审计日期：2026-07-13。
- 本轮工作基线：`858f452` (`fix(data): validate exact split keys`)。

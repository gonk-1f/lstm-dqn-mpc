...
class TestN6ExperimentContract(unittest.TestCase):
    def test_exact_candidate_set_and_horizon(self) -> None:
        from run_mpc_1s_n6_weight_selection import CANDIDATES, N6_HORIZON
...
class TestN6Metrics(unittest.TestCase):
    def test_metrics_follow_physical_energy_and_solver_status_definitions(self) -> None:
        ...
        controls = pd.DataFrame(
            {
                ...
                "P_fc_actual_kw": [110.0, 90.0, 100.0],
                "P_batt_actual_kw": [-10.0, 10.0, 0.0],
        ...
        self.assertAlmostEqual(summary["hydrogen_total_kg"], 6.0)
        self.assertAlmostEqual(summary["load_energy_mwh"], 300.0 / 3600.0 / 1000.0)
        self.assertAlmostEqual(
            summary["hydrogen_intensity_kg_per_mwh"],
            6.0 / (300.0 / 3600.0 / 1000.0),
        )
        self.assertAlmostEqual(summary["battery_charge_energy_kwh"], 10.0 / 3600.0)
        self.assertAlmostEqual(summary["battery_discharge_energy_kwh"], 10.0 / 3600.0)
        self.assertAlmostEqual(summary["battery_throughput_kwh"], 20.0 / 3600.0)
        self.assertAlmostEqual(summary["fc_above_load_fraction"], 1.0 / 3.0)
        self.assertAlmostEqual(summary["fc_surplus_energy_kwh"], 10.0 / 3600.0)
...
class TestN6Artifacts(unittest.TestCase):
    def test_candidate_artifacts_are_compact_and_self_describing(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(config["tolerances"]["fc_above_load_kw"], 1.0)
    ...
    def test_combined_report_is_manual_selection_aware(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(table["candidate_id"].tolist(), list("ABCD"))
            ...
            with self.assertRaisesRegex(ValueError, "closed-loop complete"):
                write_combined_reports(
                    ...
                    selection={
                        ...
                        "candidate_decisions": {item: "reviewed" for item in "ABCD"},
                    },
                )
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            report_path, _ = write_combined_reports(
                ...
                selection={
                    ...
                    "candidate_decisions": {item: "rejected" for item in "ABCD"},
                },
            )
    ...
    def test_report_only_cli_discovers_manual_decision_next_to_outputs(self) -> None:
        from run_mpc_1s_n6_weight_selection import main
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            (output_root / "manual_decision.json").write_text(
                json.dumps(
                    {
                        "status": "no_candidate_selected",
                        "selected_candidate": None,
                        "selection_method": "manual engineering review",
                        "selection_reasons": ["all candidates failed"],
                        "candidate_decisions": {item: "rejected" for item in "ABCD"},
                    }
                ),
                encoding="utf-8",
            ...
            report = (reports_dir / "mpc_1s_n6_weight_selection_summary.md").read_text(
                encoding="utf-8"
            )
            self.assertIn("no_candidate_selected", report)
